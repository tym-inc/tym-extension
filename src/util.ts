...
export function sendTelemetryData(eventName: string, data?: Record<string, unknown>): void {
	axios.post(
		posthogAnalyticsUrl,
		{
...
export function generateMarkdownString(description: string, terminalOutput: string, codeSnippets: ISelectionInfo[]): string {
	const codeMarkdown = codeSnippets.map(({ startLine, content, relativePath }) => {
